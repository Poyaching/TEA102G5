...
public class Class_infoJDBCDAO implements Class_info_interface {
	...
	private static final String INSERT_STMT = 
			"INSERT INTO CLASS_INFO VALUES('CLA'||LPAD(class_info_SEQ.NEXTVAL,5,'0'),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	private static final String UPDATE_STMT = 
			"UPDATE class_info set member_id=?, subclass_id=?, admin_id=?, class_name=?, class_status=?, startfund_date=?, startclass_time=?, class_description=?, class_picture=?, original_price=?, startfund_price=?, people_threshold=?, class_length=?, video_fundraising=?, update_time=? WHERE class_id=?";
	private static final String DELETE = 
			"DELETE FROM class_info where class_id= ?";
	private static final String GET_ONE_STMT = 
			"SELECT class_id, member_id, subclass_id, admin_id, class_name, class_status, startfund_date, startclass_time, class_description, class_picture, original_price, startfund_price, people_threshold, class_length, video_fundraising, update_time FROM class_info WHERE class_id= ? ";
	private static final String GET_ALL_STMT = 
			"SELECT class_id, member_id, subclass_id, admin_id, class_name, class_status, startfund_date, startclass_time, class_description, class_picture, original_price, startfund_price, people_threshold, class_length, video_fundraising, update_time FROM class_info order by class_id";
...
//				ciVO.setVideo_fundraising(rs.getBytes("video_fundraising"));
	...
	public List<Class_infoVO> get_ROWNUM_8(){
		...
		try {
			...
			Class.forName(driver);
			con = DriverManager.getConnection(url, userid, passwd);
			pstmt = con.prepareStatement("SELECT * FROM CLASS_INFO WHERE CLASS_STATUS=4 AND  ROWNUM < 9  ORDER BY CLASS_ID");
			rs = pstmt.executeQuery();
...
//				ciVO.setVideo_fundraising(rs.getBytes("video_fundraising"));
	...
	public static String get_aCondition_For_Oracle(String columnName, String value) {
		...
		if ("class_id".equals(columnName) || "member_id".equals(columnName) || 
				"class_status".equals(columnName) || "subclass_id".equals(columnName) || 
					"admin_id".equals(columnName)  )     //數字兩者相等
		...
		// 日期處理
		else if ("startfund_date".equals(columnName) || "startclass_time".equals(columnName)  || "update_time".equals(columnName))                          // 用於Oracle的date
